...
//	var itemtype = event.target.type;
	...
	$.ajax({
		...
		success: function (html) {    
			...
			$('#front-ads').html(html); 
			//$(':input[name="test_ad"]').val(html);
			if (itemtype == 'radio') $('#'+ itemid).attr('checked', 'checked').button('refresh');
			//if (itemtype == 'checkbox') $('#'+ itemid).val(itemval);         
		}
	 });
	 return false;
	});
...
//create a bitly short url for the design. Serializing is not submitting the full string. the submitted url is truncated.
$(function(){
